# Hi, I'm Siti Rasya 👋

### Sales Operations & Reporting Analyst

I have 3+ years of experience in data processing, reporting, and sales performance monitoring.

I work with operational data to create structured reports, monitor KPIs, analyze sales performance, and build dashboards to support business monitoring and decision-making.

## 🛠️ Skills

### Data & Reporting
- Data Analysis
- Data Processing
- Data Cleaning
- Data Validation
- Data Reconciliation
- Business Reporting
- Ad-hoc Reporting
- KPI Monitoring
- Sales Performance Analysis
- Dashboard Reporting
- Master Data Management

### Tools
- Microsoft Excel
- Power Query
- Power BI
- DAX
- Google Sheets
- Looker Studio

## 📊 Portfolio Projects

### 01 — Retail Sales & Customer Performance Dashboard
**Google Looker Studio | Excel | Google Sheets**

Interactive dashboard for analyzing retail sales and customer data, including:

- Revenue
- Transactions
- Average Order Value (AOV)
- Units Sold
- Monthly Revenue Trend
- Product Category Performance
- Customer Segmentation
- Basket Value Analysis

🔗 [View Project](https://github.com/sitirasyamaleka-source/retail-sales-customer-dashboard)

🔗 [View Interactive Dashboard](https://datastudio.google.com/reporting/50bcb2f1-9468-4227-9815-3df8a3450d9f)

### 02 — US Superstore Sales Performance Dashboard
**Power BI | Power Query | DAX**

Interactive Power BI dashboard for analyzing sales performance, including:

- Total Sales
- Number of Orders
- Average Order Value
- Monthly Sales Trends
- Product Categories
- Shipping Methods
- Regional Performance
- Customer Segments

The dashboard includes interactive filters for Region, Year, Segment, and Ship Mode.

## 💼 Professional Experience

**Sales Operations & Reporting**
- Data processing and reporting
- Sales performance monitoring
- KPI reporting
- Target distribution
- Master data management
- Operational data reconciliation

## 📫 Connect With Me

- LinkedIn: [Siti Rasya](https://www.linkedin.com/in/siti-rasya-maleka-prahartu-5711b52b4/)
- Email: sitirasyaa@gmail.com
